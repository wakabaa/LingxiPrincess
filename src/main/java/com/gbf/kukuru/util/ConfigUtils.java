...
import com.gbf.kukuru.bean.ConfigBean;
import com.gbf.kukuru.config.Global;
...
public class ConfigUtils {
    ...
    private static void checkConfigFileExists(boolean isReload) throws IOException {
        ...
        // Obj转为json
        JSONObject jsonObject = JSONUtil.parseObj(configBean);
        // 写入Cache File
    ...
    public static ConfigBean init(boolean isReload) {
        try {
            ...
            JSONObject configJson = JSONUtil.readJSONObject(new File(FILE_NAME), StandardCharsets.UTF_8);
            if (ObjectUtils.isEmpty(configJson)){
                throw new Exception("**文件解析失败**");
            }
            ConfigBean configBean = configJson.toBean(ConfigBean.class);
            if (isReload) {
